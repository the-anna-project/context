...
func Test_Disable_Restore(t *testing.T) {
	var val expectation.Expectation
	var ok bool
...
func Test_NewContextFromContexts(t *testing.T) {
	...
	for i, testCase := range testCases {
		...
		if testCase.ErrorMatcher == nil {
			...
			if val != nil && !val.Equals(testCase.Expected) {
...
func testAllContextsWithValue(ctxs []context.Context, val expectation.Expectation) []context.Context {
	for i, c := range ctxs {
...
func testOneContextWithValue(ctxs []context.Context, val expectation.Expectation) []context.Context {
	for i, c := range ctxs {
		ctxs[i] = NewContext(c, val)
		break
	}
...
func testNewValue(t *testing.T) expectation.Expectation {
	config := expectation.DefaultConfig()
	config.Output = "output"
	exp, err := expectation.New(config)
	if err != nil {
		t.Fatal("expected", nil, "got", err)
	}
